    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="space-y-6">
            @forelse ($orders as $order)
                {{-- Setiap pesanan adalah sebuah kartu --}}
                <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        @foreach($order->items as $item)
                            <div class="flex items-start sm:items-center justify-between">
                                <div class="flex items-start sm:items-center space-x-4">
                                    <img src="{{ asset('storage/' . $item->product->image) }}" class="w-16 h-16 object-cover rounded-md flex-shrink-0">
                                    <div>
                    ...
                    {{-- Card Footer: Rincian Total --}}
                    <div class="p-4 sm:p-6 bg-gray-50 border-t border-gray-200 text-right space-y-1 text-sm">
                        @php $subtotal = $order->items->sum(fn($item) => $item->quantity * $item->price); @endphp
                        
                        @if($order->voucher_code)
                            <div class="text-gray-600">
                                <span>Subtotal:</span>
                                <span class="font-semibold w-32 inline-block">Rp {{ number_format($subtotal) }}</span>
                            </div>
                            <div class="text-green-600">
                                <span>Diskon ({{ $order->voucher_code }}):</span>
                                <span class="font-semibold w-32 inline-block">- Rp {{ number_format($subtotal - $order->total_price) }}</span>
                            </div>
                        @endif
                        <div class="text-gray-800 font-bold text-base">
                            <span>Total:</span>
                            <span class="w-32 inline-block">Rp {{ number_format($order->total_price, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            @empty
